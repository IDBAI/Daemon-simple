...
#### 2、集成推送配置：
极光推送，信鸽推送，小米系统推送，华为系统推送，百度云推送，个推推送

##### 配置：反编译各大主流app，仿造他们的Service 和 BroadcastReceive的ACTION等配置，以及增加推送配置
##### 有效性：不受Android系统版本限制，可以在一定程度上通过隐式启动Service 或者 广播的方式 增加被唤醒的成功率
##### 测试：
1、极光推送互相唤醒
2、华为推送可以唤醒（测试发现一次）
3、百度推送可以换醒（百度网盘）
4、网络变化可以唤醒
5、android.intent.TIME_SET 可以唤醒
